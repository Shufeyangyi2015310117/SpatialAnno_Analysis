...
for (iter in 1:50){
  ...
  ## confusion index
  confusion_idx = c(sample(which(rho==1), 24),
                    sample(setdiff(1:(7*dim(rho)[1]), which(rho==1)),11))
  rho[which(rho==1)] = 0
  rho[confusion_idx] = 1
  ...
  fit_s2_Ez_u = fit_s2$Ez_u
  marker_logcount = logcounts(sce)[tolower(rownames(rho)), ]
  ...
  save(acc, f1s, kappa, err, time, ARI, CondCor, file = "~/Xingjie/simulation/simulation192_splatter_method1-3_embedding.RData")
