...
ARI <- matrix(0, 50, 10)
CondCor <- matrix(0, 50, 10)

dr_concor <- function(X, hZ, y, return_allgenes=F){
  p <- ncol(X)
  res <- lm(X~hZ);resid <- residuals(res)
  cor_p <- sapply(1:p, function(j) abs(cor(resid[,j], y)))
  if(return_allgenes){
    cor_p
  }else{
    mean(cor_p)
...
for (iter in 1:50){
  ...
  res2 = generate_count(J = 2000, de_facLoc = c(1,2,3,4,5,6,7,8)*0.1, de_facScale = c(0.1,0.1,1,0.1,1,1,0.10,0.1))
  ...
  sce <- spatialPreprocess(sce, platform="Visium", 
                           n.PCs=15, n.HVGs=2035, log.normalize=FALSE)  
  
  
  ## update rho and marker K K_unkown
  rho2 = matrix(0, dim(rho)[1] + 10, 10)
  rho2[1:dim(rho)[1], 1:7] = rho[,1:7]
  rho2[1:dim(rho)[1], 10] = rho[,8]
  rho2[(dim(rho)[1]+1):(dim(rho)[1]+5),8] = 1
  rho2[(dim(rho)[1]+6):(dim(rho)[1]+10),9] = 1
  rownames(rho2) <- c(rownames(rho), paste0("Gene", 1:10))
  colnames(rho2)[1:10] <- paste0("ct", 1:10)
  colnames(rho2)[10] <- "Unknown"
  rho = rho2
  ...
  fit_s2_Ez_u = fit_s2$Ez_u
  rownames(fit_s2_Ez_u) = paste0("spot", 1:3639)
  colnames(fit_s2_Ez_u) = paste0("gene", 1:15)
  CondCor[iter, 2] = dr_concor(as.matrix(X), fit_s2_Ez_u, y)


  PCA = reducedDim(sce,  "PCA")
  CondCor[iter, 3] = dr_concor(as.matrix(X), PCA[,1:15], y)

  
  ############DRSC  K auto select ######################################
  K_seq = 3:15
  tic <- proc.time()
  srsc <- DR.SC_fit(X, K=K_seq, Adj_sp = Adj_sp, q=15, coreNum = 5)
  drsc_out <- selectModel(srsc, criteria = 'MBIC', pen.const=1)
  srsc_lable <- drsc_out$cluster
  toc <- proc.time()
  drsc_out_hZ = drsc_out$hZ
  CondCor[iter, 4] <- dr_concor(as.matrix(X), drsc_out_hZ[,1:15], y)
  
  
  # calculate conditional correlation on integrated embeddings
  fit_s2_Ez_u = fit_s2$Ez_u
  marker_logcount = logcounts(sce)[gsub("g","G", tolower(rownames(rho))), ]
  marker_logcount_pca <- prcomp(t(marker_logcount))
  integrated_fit_s2_Ez_u = cbind(fit_s2_Ez_u, marker_logcount_pca$x[,1:15])
  rownames(integrated_fit_s2_Ez_u) = paste0("spot", 1:3639)
  colnames(integrated_fit_s2_Ez_u) = paste0("gene", 1:30)
  CondCor[iter, 9] = dr_concor(as.matrix(X), integrated_fit_s2_Ez_u[,1:30], y)
  
  
  # calculate conditional correlation on marker embeddings
  marker_logcount = logcounts(sce)[gsub("g","G", tolower(rownames(rho))), ]
  marker_logcount_pca <- prcomp(t(marker_logcount))
  marker_embedding = marker_logcount_pca$x[,1:15]
  rownames(marker_embedding) = paste0("spot", 1:3639)
  colnames(marker_embedding) = paste0("gene", 1:15)
  CondCor[iter, 10] = dr_concor(as.matrix(X), marker_embedding[,1:15], y)
  print(CondCor[iter,])
  
  
  save(acc, f1s, kappa, err, time, ARI, CondCor, file = "~/Xingjie/simulation/simulation188_splatter_method1-3_embedding.RData")
