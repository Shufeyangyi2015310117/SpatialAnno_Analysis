...
for (iter in 1:50){
  ...
  rho = rho[,c(1:4,6,8)]
  marker = marker[c(1:4,6)]
  K = dim(rho)[2] - 1
  m = dim(rho)[1]
  y2[y2=="ct5" | y2=="ct7"] = "Unknown"
  ...
  save(acc, f1s, kappa, err, time, ARI, CondCor, file = "~/Xingjie/simulation/simulation189_splatter_method1-3_embedding.RData")
