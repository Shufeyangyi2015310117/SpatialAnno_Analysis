...
for (iter in 1:50){
  ...
  save(res1, res2, file = paste0("/nas/yangyi2/Xingjie/simulation/simulation139_splatter_data", iter, ".RData"))
  ...
  save(acc, f1s, kappa, err, time, file = "/nas/yangyi2/Xingjie/simulation/simulation139_splatter_method1-3.RData")
  ...
  mat2 = mat[rownames(rho),]
  s = sizeFactors(sce)
  tic <- proc.time()
  fit <- cellassign(exprs_obj = t(mat2),
                    ...
                    verbose = FALSE)
  toc <- proc.time()
  time[iter, 6] = (toc - tic)[3]
  layer_cellassign = fit$cell_type
  index1 = which(y2!="Unknown")
  index2 = which(layer_cellassign!="other")
  index3 = intersect(index1,index2)
  err[iter, 6] = 1 - mean(layer_cellassign[index3] == as.character(y2)[index3])
  f1score = evaluate(as.character(y2)[index3], layer_cellassign[index3])
  acc[iter, 6] = f1score$Acc
  f1s[iter, 6] = mean(f1score$F1)
  kappa[iter, 6] <- cohen.kappa(x=cbind(as.character(y2)[index3], layer_cellassign[index3]))$kappa

  save(acc, f1s, kappa, err, time, file = "simulation1_method1-6.RData")
}
