...
for (iter in 1:50){
  ...
  ## confusion index
  confusion_idx = c(sample(which(rho==1), 31),
                    sample(setdiff(1:(7*dim(rho)[1]), which(rho==1)), 4))
  rho[which(rho==1)] = 0
  rho[confusion_idx] = 1
  ...
  save(acc, f1s, kappa, err, time, ARI, CondCor, file = "~/Xingjie/simulation/simulation190_splatter_method1-3_embedding.RData")
