...
for (iter in 1:50){
  ...
  seu[["pca"]] <- CreateDimReducObject(embeddings = fit_s2_Ez_u, key = "PC_", assay = DefaultAssay(seu))
  ...
  rownames(drsc_out_hZ) = paste0("spot", 1:3639)
  colnames(drsc_out_hZ) = paste0("gene", 1:15)
  seu = CreateSeuratObject(
    counts = t(drsc_out_hZ),
    project = "CreateSeuratObject",
    assay = "RNA",
    names.field = 1,
    names.delim = "_",
  ...
  seu[["pca"]] <- CreateDimReducObject(embeddings = drsc_out_hZ, key = "PC_", assay = DefaultAssay(seu))
  seu <- FindNeighbors(seu,reduction = "pca", dims = 1:15)
  seu = FindClusters(seu, resolution = 0.4)
  ARI[iter, 4] <- mclust::adjustedRandIndex(seu$seurat_clusters, y)
  print(ARI[iter,])
  
  
  ## clustering using louvain on integrated embeddings
  fit_s2_Ez_u = fit_s2$Ez_u
  ...
  seu = CreateSeuratObject(
    counts = t(integrated_fit_s2_Ez_u),
    project = "CreateSeuratObject",
    assay = "RNA",
    names.field = 1,
    names.delim = "_",
  ...
  seu[["pca"]] <- CreateDimReducObject(embeddings = integrated_fit_s2_Ez_u, key = "PC_", assay = DefaultAssay(seu))
  seu <- FindNeighbors(seu,reduction = "pca", dims = 1:30)
  seu = FindClusters(seu)
  ARI[iter, 9] = mclust::adjustedRandIndex(seu$seurat_clusters, y)
  
  
  
  
  save(acc, f1s, kappa, err, time, ARI, file = "~/Xingjie/simulation/simulation177_splatter_method1-3_embedding.RData")
